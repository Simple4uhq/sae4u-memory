...
# Default sae4u-memory home. Claude Code auto-memory dirs are discovered via glob
# at search time (see _claude_code_memory_dirs) so we don't hardcode user slugs.
DEFAULT_SAE4U_HOME = "~/.sae4u-memory"
...
def _claude_code_memory_dirs() -> list[Path]:
    """Find Claude Code auto-memory directories across all projects on this machine.

    Layout: ~/.claude/projects/<project-slug>/memory/
    The slug is generated by Claude Code from the working directory; we don't try
    to guess it — we glob.
    """
...
def resolve_roots(env_var: str = "SAE4U_MARKDOWN_ROOTS") -> list[Path]:
    """Return the list of markdown roots to search.

    Override via SAE4U_MARKDOWN_ROOTS env var (colon-separated paths).
    Defaults to all Claude Code auto-memory dirs (globbed) + sae4u-memory home.
    Missing directories are silently skipped.
    """
    raw = os.environ.get(env_var)
    if raw:
        candidates = [Path(c).expanduser() for c in raw.split(":")]
    else:
        candidates = list(_claude_code_memory_dirs())
        candidates.append(Path(DEFAULT_SAE4U_HOME).expanduser())
